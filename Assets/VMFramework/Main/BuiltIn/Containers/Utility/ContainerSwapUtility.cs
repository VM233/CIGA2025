...
namespace VMFramework.Containers
{
    public static class ContainerSwapUtility
    {
            ...
            int targetIndex)
        {
            ...
            if (thisItem != null)
            {
                if (targetContainer.IsMatchFilters(targetIndex, thisItem, out _) == false)
                {
                    return;
                }
            }
            ...
            if (targetItem != null)
            {
                if (container.IsMatchFilters(index, targetItem, out _) == false)
                {
                    return;
                }
            }
            ...
            int targetIndex)
        {
            ...
            if (targetItem == null)
            {
                container.TryPopItemBySlotIndexTo(index, targetContainer, targetIndex);
                return;
            }

            if (thisItem == null)
            {
                targetContainer.TryPopItemBySlotIndexTo(targetIndex, container, index);
                return;
            }

            if (targetContainer.TryMergeItem(targetIndex, thisItem, out _) == false)
            {
